...
document
    .getElementById("btn_non_justifie")
...
document
    .querySelector(".formulaireCacherUser")
...
enregistreUser.addEventListener("click", async function () {
    ...
    if (!nom.trim() || !username.trim() || !role.trim() || !password.trim()) {
        return alert(
            "Tous les champs de création de l'utilisateur sont obligatoire",
        );
    }
...
const chargerStatsStudent = async () => {
    ...
    document.querySelector(".statMoyenneStudent").textContent =
        stats.moyenne !== null ? `${stats.moyenne}/20` : "N/A";
};

// Recherche par nom
document
    .querySelector(".rechercheStudent input")
    .addEventListener("input", function () {
        const texte = this.value.toLowerCase();
        const resultat = tousLesStudent.filter((s) =>
...
// Filtre par classe
document
    .querySelector(".rechClasseStudent")
    .addEventListener("change", function () {
        const texte = this.value.toLowerCase();
        ...
        const resultat = tousLesStudent.filter((s) =>
            s.classe.toLowerCase().includes(texte),
        );
        afficherLignesStudents(resultat);
    });
...
rechercheStudent.addEventListener("input", function () {
    ...
    const resultat = tousLesStudent.filter(function (student) {
        return (
            (student.nom || "").toLowerCase().includes(texte) ||
            (student.matricule || "").toLowerCase().includes(texte)
        );
    });
...
// Fermer en cliquant sur le fond sombre
document
    .querySelector(".formulaireCacherStudent")
    .addEventListener("click", (e) => {
...
modifSauvStudent.addEventListener("click", async () => {
    const id = document.querySelector(".formulaireCacherStudent").dataset
        .studentId;
...
// Fermer en cliquant sur le fond sombre
document
    .querySelector(".formulaireCacherProf")
    .addEventListener("click", (e) => {
...
const statMatiereNonAssignee = document.querySelector(
    ".statMatiereNonAssignee",
);
...
// Fermer en cliquant sur le fond sombre
document
    .querySelector(".formulaireCacherMatiere")
    .addEventListener("click", (e) => {
...
modifSauvMatiere.addEventListener("click", async () => {
    const id = document.querySelector(".formulaireCacherMatiere").dataset
        .subjectId;
...
// RECUPERATION DE LA SECTION NOTES
...
const chargerNotes = async () => {
    ...
    afficherLignesNotes(tousLesNotes);
};
...
const chargerStatsNotes = async () => {
    ...
    document.querySelector(".statTotalNotes").textContent = stats.total;
    document.querySelector(".statMoyenneNotes").textContent =
        stats.moyenne !== null ? `${stats.moyenne}/20` : "N/A";
    document.querySelector(".statNotesDessous").textContent = stats.enDessous;
};
...
const chargerMatieresNote = async () => {
...
const chargerClassesNote = async () => {
    ...
    const select = document.getElementById("classe_note");
...
// Charger les étudiants en fonction de la classe choisie


const classeEtudiant = document.getElementById("classe_note");

classeEtudiant.addEventListener("change", async function () {

    const classe = this.value;
    const token = localStorage.getItem("token");

    const select = document.getElementById("noteEtudiant");
...
// Recherche par nom etudiant
const rechercheNote = document.querySelector(".rechercheNote input");

rechercheNote.addEventListener("input", function () {
    const texte = this.value.toLowerCase();
    const resultat = tousLesNotes.filter(function (note) {
        return (
            (note.nom || "").toLowerCase().includes(texte) ||
            (note.prenom || "").toLowerCase().includes(texte)
        );
    });
    afficherLignesNotes(resultat);
});

// Afficher les lignes des notes
const afficherLignesNotes = (liste) => {
    const tbody = document.getElementById("listeNotes");
    ...
    liste.forEach(function (grade) {
        ...
        // Modifier — ouvre le formulaire caché pré-rempli
        btnModif.addEventListener("click", () => {
            document.getElementById("modifValeurNote").value = grade.note;

            const overlay = document.querySelector(".formulaireCacherNote");
...
// Boutons du formulaire caché de modification
const AnnulerNote = document.getElementById("AnnulerNote");
const modifSauvNote = document.getElementById("modifSauvNote");
const btnFermNote = document.querySelector(".btn_fermNote");

// Fermer sans sauvegarder
AnnulerNote.addEventListener("click", () => {
    document.querySelector(".formulaireCacherNote").style.display = "none";
});

btnFermNote.addEventListener("click", () => {
    document.querySelector(".formulaireCacherNote").style.display = "none";
});

// Fermer en cliquant sur le fond sombre
document
    .querySelector(".formulaireCacherNote")
    .addEventListener("click", (e) => {
        if (e.target.classList.contains("formulaireCacherNote")) {
            document.querySelector(".formulaireCacherNote").style.display = "none";
        }
    });

// Sauvegarder la modification
modifSauvNote.addEventListener("click", async () => {
    const id = document.querySelector(".formulaireCacherNote").dataset.noteId;
...
// Ajouter une note
const btn_enregistreNote = document.getElementById("btn_enregistreNote");

btn_enregistreNote.addEventListener("click", async function () {
    const token = localStorage.getItem("token");
    const student_id = document.getElementById("noteEtudiant").value;
    const subject_id = document.getElementById("noteMatiere").value;
    const note = document.getElementById("noteValeur").value;

    if (!student_id || !subject_id || !note) {
    ...
    document.getElementById("noteEtudiant").value = "";
...
// Bouton annuler du formulaire d'ajout
const btnAnnulNote = document.querySelector(".btnAnnulNote");

btnAnnulNote.addEventListener("click", function () {
    document.getElementById("noteEtudiant").value = "";
    document.getElementById("noteMatiere").value = "";
    document.getElementById("noteValeur").value = "";
});
